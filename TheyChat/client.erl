- module(client).
- export([start/1, list_servers_from_router/1, join_server/1, send_message/1]).

start([ClientName, Router]) ->
    connect_with_router(Router, 3),
    register(ClientName, spawn(fun() -> listen(ClientName, Router, []) end)).
...
listen(ClientName, Router, Servers) -> 
    receive
        ...
        {join_server, Pid, Topic} ->
            ...
            receive
                {router, [{ServerPid, _}]} ->
                    ServerPid ! {add_client, self(), ClientName},
                    receive
                        {ServerPid, ok} -> 
        ...
        {send_message, Pid, Topic, Message} ->
            ...
            case FindTopic of
                ...
                [{ServerPid, _}] -> ServerPid ! {message, self(), ClientName, Message}
            end;
